...
function surface(root, canvas, spec) {
  ...
  const seg = trail.geometry.attributes.instanceStart.data;
  scene.add(trail);
  ...
  function render() {
    ...
    // Only the heights move (the surface is still rising); x and z were written
    // once. instanceCount, never a resize, decides how much of the path shows.
    for (let i = 0; i < path.length - 1; i++) {
      seg.array[i * 6 + 1] = path[i].y * g + 0.08;
      seg.array[i * 6 + 4] = path[i + 1].y * g + 0.08;
    }
    seg.needsUpdate = true;
    trail.geometry.instanceCount = n;
